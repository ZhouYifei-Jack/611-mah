...
public class Heroandmonstergame extends Quoridor{
    ...
    public void init() throws IOException {
        ...
        System.out.println("Here is the list of heroes and their starting statistics. You make pick " + 1 + " unique heroes.");
        ...
        for (int i = 1; i <= heroList.length; i++) {
            System.out.print(i + ". ");
            System.out.println(heroList[i-1].get_name()+" "+heroList[i-1].getMana()+" "+heroList[i-1].getStrength()+" "+heroList[i-1].getAgility()+" "+heroList[i-1].getDexterity()+" "+heroList[i-1].getStarting_money()+" "+heroList[i-1].getStarting_experience()+" "+heroList[i-1].getType());
        }
        System.out.println("Now pick a hero,please:");
        int c_h=this.CheckSetUp(1,heroList.length+1);
        System.out.println(heroList[c_h-1].get_name()+" "+heroList[c_h-1].getMana()+" "+heroList[c_h-1].getStrength()+" "+heroList[c_h-1].getAgility()+" "+heroList[c_h-1].getDexterity()+" "+heroList[c_h-1].getStarting_money()+" "+heroList[c_h-1].getStarting_experience());

        Warrior warrior=new Warrior(heroList[c_h-1].get_name(),heroList[c_h-1].getMana(),heroList[c_h-1].getStrength(),heroList[c_h-1].getAgility(),heroList[c_h-1].getDexterity(),heroList[c_h-1].getStarting_money(),heroList[c_h-1].getStarting_experience());
        ...
        while (!is_move) {
                System.out.println("hi," + this.current_player.get_name() +
                        ...
                        "\n3 for left" +
                        "\n4 for right");
                int c = this.CheckSetUp(1, 4);



                if (check_direction(c, current_player.getPlayer_current_row(), current_player.getPlayer_current_col())) {
