...
public class Market extends Pieces{
    ...
    public void shopping(Hero hero) throws IOException {
        ...
        System.out.println("Now pick a grocery,please(enter No. for shopping,enter 0 for quit):");
        Generalboardgame c=new Generalboardgame();
        int p_g=c.CheckSetUp(0,storage.size());
        while(p_g!=0){
            hero.buygrocery(storage.get(p_g-1));
            System.out.println("Now pick a grocery,please(enter No. for shopping,enter 0 for quit):");
            p_g=c.CheckSetUp(0,storage.size());
        }
